...
const Cart = () => {
  ...
  const columns: ColumnsType<ItemInCart> = [
    ...
    },
    {
      ...
      render: (item) => (
        <div>
          <NavLink to={`/sneakers-item/${item.id}`}>{item.title}</NavLink>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      key: "quantity",
      dataIndex: "quantity",
      render: (quantity, item) => <div>{item.item.price * quantity} USD</div>,
    },
    {
      title: "Remove",
      key: "action",
      render: (_, item) => (
        <Button
          onClick={() =>
            dispatch(
              removeItem({ productSize: item.productSize, id: item.item.id })
            )
          }
        >
          X
  ...
  return cart ? (
    <div className={st.cart}>
      <div className={st.cart_container}>
        ...
        <div className={st.table_delivery_container}>
